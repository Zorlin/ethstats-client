...
function Node ()
{
	...
	this.startDw3bConnection();

	return this;
}

Node.prototype.startDw3bConnection = function()
{
	console.info('Starting dw3b connection');

	dw3b = new Dw3b();
	dw3b.setProvider(new dw3b.providers.HttpProvider((process.env.RPC_PROTOCOL || 'http') + '://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '1234')));

	console.info('Checking dw3b connection')
	this.checkDw3bConnection();
}

Node.prototype.checkDw3bConnection = function()
{
	var self = this;
	...
	{
		if(dw3b.isConnected()) {
			console.success('Dw3b connection established');
		...
		else
		{
			...
			{
				console.error('Dw3b connection attempt', chalk.cyan('#' + this._connection_attempts++), 'failed');
				console.error('Trying again in', chalk.cyan(500 * this._connection_attempts + ' ms'));

				setTimeout(function ()
				{
					self.checkDw3bConnection();
				}, CONNECTION_ATTEMPTS_TIMEOUT * this._connection_attempts);
			}
			else
			{
				console.error('Dw3b connection failed', chalk.cyan(MAX_CONNECTION_ATTEMPTS), 'times. Aborting...');
			}
		}
	}
}
...
Node.prototype.reconnectWeb3 = function()
{
	...
	{
		console.error("Dw3b reset error:", err);
	}

	console.info("Dw3b reconnect attempts started");
...
Node.prototype.getInfo = function()
{
	...
	try {
		...
		// }
		// Version of the client (Lotus node)
		this.info.node = dw3b.version.node;
		// Version of the network
		this.info.net = dw3b.version.network;
		// Current protocol version
		this.info.protocol = dw3b.toDecimal(web3.version.ethereum);
		// Current API version
		this.info.api = dw3b.version.api;
